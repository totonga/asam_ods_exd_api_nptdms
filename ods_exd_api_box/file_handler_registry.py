...
class FileHandlerRegistry:
    ...
    def register(cls,
        ...
        cls.log.info("Registering file handler for type '%s' with patterns %s",
                     file_type_name, patterns)
        cls._handlers[file_type_name] = FileType(
            file_type_name, patterns, factory)
        cls.log.debug(
            "File handler registered successfully for type '%s'", file_type_name)
    ...
    def create(cls, file_type_name: str, file_path: str, parameters: str) -> ExdFileInterface:
        ...
        """
        cls.log.debug("Creating handler for file type '%s', file path '%s', parameters '%s'",
                      file_type_name, file_path, parameters)
        ...
        if file_type is None:
            cls.log.error("File type '%s' is not registered. Available types: %s",
                          file_type_name, list(cls._handlers.keys()))
            raise ValueError(f"File type '{file_type_name}' is not registered")
        if file_type.factory is None:
            cls.log.error(
                "No factory registered for file type '%s'", file_type_name)
            raise ValueError(
                f"No factory registered for file type '{file_type_name}'")
        cls.log.info(
            "Creating handler instance for file type '%s'", file_type_name)
    ...
    def get_file_type_name(cls, file_path: str) -> str:
      ...
      """
        ...
        if len(cls._handlers) == 1:
            ...
            cls.log.debug(
                "Only one handler registered, using type '%s'", file_type)
            return file_type

        # Extract filename using Path
        filename = Path(file_path).name.upper()
        cls.log.debug(
            "Matching filename '%s' against registered patterns", filename)
        ...
        for file_type_name, file_type_info in cls._handlers.items():
            for pattern in file_type_info.file_patterns:
                if fnmatch(filename, pattern.upper()):
                    cls.log.info("Matched file type '%s' for file '%s' using pattern '%s'",
                                 file_type_name, filename, pattern)
                    return file_type_name

        cls.log.error(
            "No handler registered for file pattern matching '%s'", filename)
        raise ValueError(
    ...
    def create_from_path(cls, file_path: str, parameters: str) -> ExdFileInterface:
        ...
        cls.log.debug(
            "Detected file type '%s', creating handler", file_type_name)
