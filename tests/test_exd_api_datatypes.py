    ...
    def test_datatype(self):
        with tempfile.TemporaryDirectory() as temporary_directory_name:
            file_path = os.path.join(
                temporary_directory_name, "all_datatypes.tdms")

            with TdmsWriter(file_path) as tdms_writer:
                tdms_writer.write_segment([ChannelObject(
                ...
                tdms_writer.write_segment([ChannelObject("group_date", "date_data", np.array(
                    [datetime(2017, 7, 9, 12, 35, 0), datetime(2017, 7, 9, 12, 36, 0)], np.datetime64))])

                tdms_writer.write_segment(
                    [ChannelObject("group_real", "float32_data", np.array([1.1, 1.2], np.float32))])
                tdms_writer.write_segment(
                    [ChannelObject("group_real", "float64_data", np.array([2.1, 2.2], np.float64))])
            ...
            try:
                ...
                self.assertEqual(
                    structure.groups[0].channels[0].data_type, ods.DataTypeEnum.DT_COMPLEX)
                self.assertEqual(
                    structure.groups[0].channels[1].data_type, ods.DataTypeEnum.DT_DCOMPLEX)

                self.assertEqual(
                    structure.groups[1].channels[0].data_type, ods.DataTypeEnum.DT_SHORT)
                self.assertEqual(
                    structure.groups[1].channels[1].data_type, ods.DataTypeEnum.DT_BYTE)
                self.assertEqual(
                    structure.groups[1].channels[2].data_type, ods.DataTypeEnum.DT_SHORT)
                self.assertEqual(
                    structure.groups[1].channels[3].data_type, ods.DataTypeEnum.DT_LONG)
                self.assertEqual(
                    structure.groups[1].channels[4].data_type, ods.DataTypeEnum.DT_LONG)
                self.assertEqual(
                    structure.groups[1].channels[5].data_type, ods.DataTypeEnum.DT_LONGLONG)
                self.assertEqual(
                    structure.groups[1].channels[6].data_type, ods.DataTypeEnum.DT_LONGLONG)
                self.assertEqual(
                    structure.groups[1].channels[7].data_type, ods.DataTypeEnum.DT_DOUBLE)

                self.assertEqual(
                    structure.groups[2].channels[0].data_type, ods.DataTypeEnum.DT_DATE)

                self.assertEqual(
                    structure.groups[3].channels[0].data_type, ods.DataTypeEnum.DT_FLOAT)
                self.assertEqual(
                    structure.groups[3].channels[1].data_type, ods.DataTypeEnum.DT_DOUBLE)
                ...
                self.assertSequenceEqual(
                    values.channels[0].values.float_array.values, [1.0, 2.0, 3.0, 4.0])
                self.assertEqual(
                    values.channels[1].values.data_type, ods.DataTypeEnum.DT_DCOMPLEX)
                self.assertSequenceEqual(
                ...
                self.assertSequenceEqual(
                    values.channels[0].values.string_array.values, ['abc', 'def'])
