        ...
        with tempfile.TemporaryDirectory() as temporary_directory_name:
            ...
            try:
                ...
                values = service.GetValues(
                    exd_api.ValuesRequest(handle=handle, group_id=0, start=0, limit=2, channel_ids=[0, 1]),
                    None,
                )
                ...
                values = service.GetValues(
                    exd_api.ValuesRequest(
                        handle=handle, group_id=1, start=0, limit=2, channel_ids=[0, 1, 2, 3, 4, 5, 6, 7]
                    ),
                    None,
                )
                ...
                values = service.GetValues(
                    exd_api.ValuesRequest(handle=handle, group_id=2, start=0, limit=2, channel_ids=[0]), None
                )
                ...
                values = service.GetValues(
                    exd_api.ValuesRequest(handle=handle, group_id=3, start=0, limit=2, channel_ids=[0, 1]),
                    None,
                )
                self.assertEqual(values.channels[0].values.data_type, ods.DataTypeEnum.DT_FLOAT)
                self.assertSequenceEqual(
                    values.channels[0].values.float_array.values, [1.100000023841858, 1.2000000476837158]
                )
                ...
                values = service.GetValues(
                    exd_api.ValuesRequest(handle=handle, group_id=4, start=0, limit=2, channel_ids=[0]), None
                )
