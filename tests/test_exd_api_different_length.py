    ...
    def test_different_length(self):
        with tempfile.TemporaryDirectory() as temporary_directory_name:
            ...
            with TdmsWriter(file_path) as tdms_writer:
                ...
                tdms_writer.write_segment(
                    [ChannelObject("group_1", "channel_4", np.array([4.1, 4.2, 4.3, 4.4], np.float64))]
                )

                tdms_writer.write_segment(
                    [ChannelObject("group_2", "channel_1", np.array([1.1, 1.2, 1.3, 1.4], np.float64))]
                )
                tdms_writer.write_segment(
                ...
                tdms_writer.write_segment(
                    [ChannelObject("group_4", "channel_2", np.array([2.1, 2.2, 2.3, 2.4], np.float64))]
                )
                tdms_writer.write_segment(
                    [ChannelObject("group_4", "channel_3", np.array([3.1, 3.2, 3.3, 3.4], np.float64))]
                )
                tdms_writer.write_segment(
                    [ChannelObject("group_4", "channel_4", np.array([4.1, 4.2, 4.3, 4.4], np.float64))]
                )
            ...
            try:
                ...
                values = service.GetValues(
                    exd_api.ValuesRequest(
                        handle=handle,
                        group_id=meta_group.id,
                        start=0,
                        limit=4,
                        channel_ids=[meta_group.channels[0].id, meta_group.channels[1].id],
                    ),
                    None,
                )
                ...
                values = service.GetValues(
                    exd_api.ValuesRequest(
                        ...
                        limit=4,
                        channel_ids=[
                            meta_group.channels[0].id,
                            meta_group.channels[1].id,
                            meta_group.channels[2].id,
                            meta_group.channels[3].id,
                        ],
                    ),
                    None,
                )
